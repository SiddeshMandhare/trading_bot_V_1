...
#####################################################################################################
    ...
    def save_config(self):
        """Save configuration to config.py - FIXED for type annotations"""
        try:
            length = int(self.headers.get('Content-Length', 0))
            ...
            print(f"{'='*60}")
            print(f"Keys received: {list(data.keys())[:10]}...")
            ...
            for key, value in data.items():
                # Skip if key is not a config variable
                ...
                # Pattern to match with type annotations (e.g., "KEY: type = value")
                # Match both with and without type annotations
                patterns = [
                    rf'^{key}\s*=\s*[^\n]+',                    # KEY = value
                    rf'^{key}\s*:\s*\w+\s*=\s*[^\n]+',          # KEY: type = value
                    rf'^{key}\s*:\s*\w+\s*=\s*[^\n]+\s*#.*',    # KEY: type = value # comment
                    rf'^{key}\s*=\s*[^\n]+\s*#.*',              # KEY = value # comment
                ]
                ...
                for pattern in patterns:
                    if re.search(pattern, content, re.MULTILINE):
                        ...
                        import re
                        line_match = re.search(rf'^{key}.*$', content, re.MULTILINE)
                        if line_match:
                            ...
                            type_match = re.search(r':\s*\w+\s*=', original_line)
                            if type_match:
                                # Has type annotation, preserve it
                                type_str = type_match.group(0).strip()
                                new_line = f'{key} {type_str} {value_str} {comment}'.strip()
                            else:
                                new_line = f'{key} = {value_str} {comment}'.strip()
                            
                            content = content.replace(original_line, new_line)
                            found = True
                            updated_count += 1
                            print(f"  ✅ Updated: {key} = {value_str}")
                            break
                
                if not found:
                    # Try without spaces around =
                    pattern_no_space = rf'^{key}\s*:\s*\w+\s*=\s*[^\n]+'
                    if re.search(pattern_no_space, content, re.MULTILINE):
                        content = re.sub(pattern_no_space, f'{key}: {type(value).__name__} = {value_str}', content, flags=re.MULTILINE)
                        updated_count += 1
                        print(f"  ✅ Updated (no-space): {key}")
                    else:
            ...
            print(f"📝 Updated {updated_count} configuration settings")
            if failed_keys:
                print(f"⚠️ Failed to update: {failed_keys}")
            print(f"{'='*60}\n")
            
            # Also update runtime config if needed
            self._update_runtime_config(data)
            
            # Write to log file
            self._log_config_change(data, updated_count)
