...
class Tradehull:    
    ...
    def __init__(self, ClientCode: str, token_id: str = "", mode: str = "access_token", **login_kwargs):
        ...
        mode                                    = "access_token", "api_key", or "pin_totp"
        '''
        ...
        # Initialize instrument_df as empty DataFrame initially
        ...
        try:
            ...
            if not login_ok:
                print("⚠️ Login failed, but continuing with limited functionality")
                # instrument_df already set to empty DataFrame
            else:
                print("-----SUCCESSFULLY LOGGED INTO DHAN-----")
            
            # Ensure instrument_df exists (should be set by get_login, but double-check)
            if not hasattr(self, 'instrument_df') or self.instrument_df is None:
                print("⚠️ instrument_df not set, creating empty DataFrame")
                self.instrument_df = pd.DataFrame()
            ...
            print(e)
            traceback.print_exc()
            # Ensure instrument_df exists even on exception
            if not hasattr(self, 'instrument_df') or self.instrument_df is None:
                self.instrument_df = pd.DataFrame()
    ...
    def get_login(self, ClientCode: str, token_id: str = "", mode: str = "access_token", **kwargs) -> bool:
        ...
        try:
            ...
            # Helper function to ensure instrument_df is loaded
            def ensure_instrument_df():
                try:
                    if not hasattr(self, 'instrument_df') or self.instrument_df is None or self.instrument_df.empty:
                        print("📂 Loading instrument file...")
                        self.instrument_df = self.get_instrument_file()
                        if self.instrument_df is not None and not self.instrument_df.empty:
                            print(f"✅ Instrument file loaded: {len(self.instrument_df)} records")
                        else:
                            print("⚠️ Instrument file is empty, creating fallback")
                            self.instrument_df = pd.DataFrame()
                    return True
                except Exception as e:
                    print(f"⚠️ Failed to load instrument file: {e}")
                    self.instrument_df = pd.DataFrame()
                    return False
            ...
            if mode == "access_token":
                ...
                try:
                    ...
                    if not new_tok:
                        print("⚠️ Renew failed: token not found in response.")
                        # Try using the token as-is
                        self.dhan_context = DhanContext(self.ClientCode, self.token_id)
                        self.Dhan = dhanhq(self.dhan_context)
                        ensure_instrument_df()
                        print("✅ Using provided token without renewal")
                        return True
                    ...
                    print(f"⚠️ Token renew failed: {e}")
                    # Try using the original token anyway
                    try:
                        self.dhan_context = DhanContext(self.ClientCode, self.token_id)
                        self.Dhan = dhanhq(self.dhan_context)
                        ensure_instrument_df()
                        print("✅ Using provided token (renewal skipped)")
                        return True
                    except Exception as e2:
                        print(f"❌ Failed to initialize with provided token: {e2}")
                        self.instrument_df = pd.DataFrame()
                        return False
            
            # -----------------------------
            # 2) API KEY MODE (OAuth)
            # -----------------------------
            elif mode == "api_key":
                ...
                for attempt in range(max_attempts):
                    user_input = input(f"Paste redirect URL (attempt {attempt + 1}/{max_attempts}): ").strip()
                    if not user_input:
                    ...
                    if not extracted_token_id:
                        print("⚠️ Could not find tokenId in what you pasted. Please try again.")
                        continue
                    ...
                    try:
                        ...
                        print(f"❌ Failed to consume token: {e}")
                        continue
            ...
            # -----------------------------
                ...
                pin = kwargs.get("pin")
                if not pin:
                    print("❌ PIN is required for pin_totp mode")
                    self.instrument_df = pd.DataFrame()
                    return False
                ...
                if totp_secret:
                    try:
                        ...
                        print(f"⚠️ Failed to generate TOTP from secret: {e}")
                        totp = kwargs.get("totp") or input("Enter current TOTP (6-digit): ").strip()
                else:
                    totp = kwargs.get("totp") or input("Enter current TOTP (6-digit): ").strip()
                
                if not totp:
                    print("❌ TOTP is required")
                    self.instrument_df = pd.DataFrame()
                    return False
                ...
                raise ValueError(f"Invalid mode: {mode}. Use 'access_token', 'api_key', or 'pin_totp'")
            ...
            traceback.print_exc()
            # Ensure instrument_df exists even on failure
            self.instrument_df = pd.DataFrame()
            return False

    def _extract_token_id_from_input(self, text: str) -> str:
        """Extract tokenId from redirect URL"""
        try:
    ...
    def _read_token_today(self) -> str:
        ...
        try:
            ...
            print(f"⚠️ Error reading token cache: {e}")
        return ""
    
    def _save_token_today_once(self, token: str):
        """Save token for today (only once per day)"""
        try:
            p = self._token_path_today()
            if os.path.exists(p):
                return
    ...
    def _delete_all_token_files(self):
        ...
        try:
            ...
            print("🗑️ All token cache files deleted")
        except Exception as e:
            print(f"⚠️ Error deleting token files: {e}")
    ...
    def convert_to_date_time(self,epoch):
        ...
        dt = datetime.datetime.fromtimestamp(epoch, IST)
    ...
    def get_historical_data(self,tradingsymbol,exchange,timeframe, sector = "NO",debug="NO"):            
        try:
            ...
            if ohlc['status']!='failure':
                ...
                if not df.empty:
                    df['timestamp'] = df['timestamp'].apply(lambda x: self.convert_to_date_time(x))
                    print(f"   ✅ DHAN returned {len(df)} rows for {tradingsymbol}")
                    return df
                else:
                    print(f"   ⚠️ DHAN returned empty DataFrame for {tradingsymbol}")
                    return df
            else:
                print(f"   ❌ DHAN API failed: {ohlc}")
    ...
    def order_report(self) -> Tuple[Dict, Dict, Dict]:
        '''
        ...
        try:
            ...
            if not status_df.empty:
                ...
                order_placed_time = status_df['exchangeTime'].to_dict()
            return order_details, order_exe_price, order_placed_time
        except Exception as e:
    ...
    def get_executed_price(self, orderid:str, debug= "NO")->float:
        try:
            if orderid is None:
                raise Exception('Check the order id, Error as None')            
            orderid = str(orderid)
    ...
    def get_executed_price_and_time(self, orderid:str, debug= "NO")->Tuple[float, str]:
        try:
            if orderid is None:
                raise Exception('Check the order id, Error as None')            
            orderid = str(orderid)
            ...
            if debug.upper()=="YES":
                print(response)                
            if response['status']=='success':
                executed_price = response['data'][0]['averageTradedPrice']
                exchange_time = response['data'][0]['exchangeTime']
                return executed_price, exchange_time
            else:
                raise Exception(response)
        except Exception as e:
            print(f"Error at get_executed_price_and_time as {e}")
            return {
                'status':'failure',
                'remarks':str(e),
                'data':response,
    ...
    def get_expiry_date(self, Underlying, opt_fut):
        try:
            instrument = instrument_df.copy()
            instrument["Underlying"] = instrument["SEM_TRADING_SYMBOL"].str.split("-", n=1).str[0]
            instrument['SEM_EXPIRY_DATE'] = pd.to_datetime(instrument['SEM_EXPIRY_DATE'], errors='coerce')
            instrument['ContractExpiration'] = instrument['SEM_EXPIRY_DATE'].dt.date
            instrument['ContractExpiration'] = instrument['ContractExpiration'].astype(str)
            ...
            script_exchange = {"NSE":self.Dhan.NSE, "NFO":self.Dhan.FNO, "BFO":"BSE_FNO", "CUR": self.Dhan.CUR, "BSE":self.Dhan.BSE, "MCX":self.Dhan.MCX, "INDEX":self.Dhan.INDEX}
            instrument_exchange = {'NSE':"NSE",'BSE':"BSE",'NFO':'NSE','BFO':'BSE','MCX':'MCX','CUR':'NSE'}
            index_exchange = {"NIFTY":'NSE',"BANKNIFTY":"NSE","FINNIFTY":"NSE","MIDCPNIFTY":"NSE","BANKEX":"BSE","SENSEX":"BSE","NIFTYNXT50":"NSE", "MCXBULLDEX":"MCX"}
            if Underlying in index_exchange:
                exchange =index_exchange[Underlying]
                if opt_fut.upper() == "OPTION":
                    dates_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "OPTIDX") &(instrument["Underlying"] == Underlying)]
                if opt_fut.upper() == "FUTURE":
                    dates_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "FUTIDX") &(instrument["Underlying"] == Underlying)]
            elif Underlying in self.commodity_step_dict.keys():
                exchange = "MCX"
                if opt_fut.upper() == "OPTION":
                    dates_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "OPTFUT") &(instrument["Underlying"] == Underlying)]
                if opt_fut.upper() == "FUTURE":
                    dates_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "FUTCOM") &(instrument["Underlying"] == Underlying)]
            elif Underlying in self.stock_step_df:
                exchange = "NSE"
                if opt_fut.upper() == "OPTION":
                    dates_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "OPTSTK") &(instrument["Underlying"] == Underlying)]
                if opt_fut.upper() == "FUTURE":
                    dates_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "FUTSTK") &(instrument["Underlying"] == Underlying)]                
            else:
                return []
            ...
            dates = sorted( dates_df['ContractExpiration'].unique())
            return dates
        except Exception as e:
            print(f"Exception at getting Expiry dates as {e}")
            return list()

    def get_future_script(self, underlying, expiry):
        try:
            instrument = instrument_df.copy()
            instrument["Underlying"] = instrument["SEM_TRADING_SYMBOL"].str.split("-", n=1).str[0]
            instrument['SEM_EXPIRY_DATE'] = pd.to_datetime(instrument['SEM_EXPIRY_DATE'], errors='coerce')
            ...
            if Underlying in index_exchange:
                exchange =index_exchange[Underlying]
                future_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "FUTIDX") &(instrument["Underlying"] == Underlying)].sort_values('ContractExpiration')
            elif Underlying in self.commodity_step_dict.keys():
                exchange = "MCX"
                future_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "FUTCOM") &(instrument["Underlying"] == Underlying)].sort_values('ContractExpiration')
            elif Underlying in self.stock_step_df:
                exchange = "NSE"
                future_df = instrument[(instrument["SEM_EXM_EXCH_ID"] == exchange) &(instrument["SEM_INSTRUMENT_NAME"] == "FUTSTK") &(instrument["Underlying"] == Underlying)].sort_values('ContractExpiration')
            else:
                return None
            ...
            if len(future_df)<=int(expiry):
                expiry=len(future_df)-1
            future_script = future_df.iloc[int(expiry)]['SEM_CUSTOM_SYMBOL']            
            return future_script
        except Exception as e:
            print(f"Exception at getting Future script as {e}")
            return None
