...
class Tradehull:    
	...
	def convert_to_date_time(self, epoch):
		...
		try:
			...
			self.logger.exception(f"Error converting epoch {epoch}: {e}")
			return epoch

	def get_start_date(self):
		try:
			instrument_df = self.instrument_df.copy()
			# Reduce from 90 days to 30 days to avoid rate limits
			...
			ohlc = self.Dhan.historical_daily_data(int(security_id), exchange_segment, instrument_type, from_date_str, to_date, int(expiry_code))
