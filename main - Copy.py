...
##            os.remove('trading_bot.db')
    ...
    def set_trading_mode(self, mode: str):
        """Change trading mode on the fly"""
        from config import Config
        from config import Config
        ...
        if mode.upper() in ['EQUITY_ONLY', 'FNO_ONLY', 'BOTH']:
            ...
            # Auto-configure option trading based on mode
            if mode.upper() == 'EQUITY_ONLY':
                option_config.OPTION_TRADING_ENABLED = False
                print(f"✅ Trading mode set to: {mode.upper()} (Equity only)")
            elif mode.upper() == 'FNO_ONLY':
                option_config.OPTION_TRADING_ENABLED = True
                print(f"✅ Trading mode set to: {mode.upper()} (F&O only)")
            else:
                option_config.OPTION_TRADING_ENABLED = True
                print(f"✅ Trading mode set to: {mode.upper()} (Both Equity & F&O)")
            
            # Send Telegram alert
            self.telegram.send_alert(f"🔄 Trading Mode Changed to: {mode.upper()}")
            return True
        return False
